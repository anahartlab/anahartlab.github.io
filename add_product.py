...
gallery_folder = input("Имя папки с изображениями в 'images/': ").strip()

gallery_path = os.path.join("images", gallery_folder)

if not os.path.exists(gallery_path) or not os.path.isdir(gallery_path):
    print(f"❌ Папка '{gallery_path}' не найдена.")
    exit()

# Список картинок с нужными расширениями
valid_extensions = {".jpg", ".jpeg", ".png"}
gallery_images = [f for f in sorted(os.listdir(gallery_path))
                  if os.path.splitext(f.lower())[1] in valid_extensions]

if not gallery_images:
    print(f"❌ В папке '{gallery_path}' нет изображений с расширениями jpg, jpeg, png.")
    exit()

# Формируем HTML для галереи
carousel_indicators = ""
carousel_items = ""
...
for i, img_name in enumerate(gallery_images):
    ...
    <div class="{active_class} u-carousel-item u-gallery-item u-carousel-item-{i+1}">
      <div class="u-back-slide" data-image-width="960" data-image-height="1280">
        <img class="u-back-image u-expanded" src="images/{gallery_folder}/{img_name}">
      </div>
...
# Шаблон блока с динамической галереей
product_block = f"""
<section class="u-clearfix u-section-16" id="pareo_flow_msk">
            ...
            <div class="u-layout-col">
              <div class="u-container-style u-layout-cell u-size-60 u-layout-cell-2">
                <div class="u-container-layout u-container-layout-2">
                  ...
                  <a href="{link}" class="u-btn u-btn-round u-button-style u-custom-font u-heading-font u-hover-palette-1-light-1 u-palette-1-base u-radius u-btn-1" title="Оплатить">Оплатить</a>
                </div>
              </div>
            </div>
          </div>
...
insert_index = content.lower().find("<footer")
if insert_index == -1:
    print("❌ Не удалось найти <footer> для вставки.")
    exit()
...
print("✅ Новый товар добавлен в PSYWEAR.html перед <footer>")